...
from embedding import MSTE
from kgetools import KgDataset
import sys

sys.path.append("..")
from kgetools.metrics.ranking import precision_at_k, average_precision, APN
...
def main():
    ...
    benchmark_triples = np.array([[d1, se, d2] for d1, d2, se in
    ...
    def so_sort(benchmark_triples):
        tri = []
        for s, o, p in benchmark_triples:
            if s > o:
                tri.append((s, p, o))
            else:
                tri.append((o, p, s))
    ...
    model = MSTE(seed=seed, verbose=2)
    ...
    metrics_per_se = {se_idx: {"ap": .0, "auc-roc": .0, "auc-pr": .0, "AP@n": .0} for se_idx in pse_indices}
    ...
    for se in tqdm(pse_indices, desc="Evaluating test data for each side-effect"):
        ...
        print("AP: %1.4f - AUC-ROC: %1.4f - AUC-PR: %1.4f - AP@n: %1.4f ---------> %s" %
              (se_ap, se_auc_roc, se_auc_pr, se_pn, se_code), flush=True)
